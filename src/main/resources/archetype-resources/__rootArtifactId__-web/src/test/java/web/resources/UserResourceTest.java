...
class UserResourceTest {
    ...
    void findByIdExpectResult() throws Exception {
        ...
        mockMvc.perform(get("/api/v1/users/1"))
                .andExpectAll(
                        status().isOk(),
                        jsonPath("${symbol_dollar}.id").value(1L),
                        jsonPath("${symbol_dollar}.name").value("Giacomo"),
                        jsonPath("${symbol_dollar}.surname").value("Chiavolotti"),
                        jsonPath("${symbol_dollar}.email").value("c.giacomo@hotmail.it"),
                        jsonPath("${symbol_dollar}.roles[0]").value("ADMIN")
                );
    }
